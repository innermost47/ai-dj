...
void PromptBankPanel::resized()
{
	...
	searchInput.setBounds(area.removeFromTop(24).reduced(0, 2));
	area.removeFromTop(4);

	sortMenu.setBounds(area.removeFromTop(24).reduced(0, 2));
	area.removeFromTop(4);

	auto createRow = area.removeFromTop(28).reduced(0, 2);
	...
	auto expandRow = area.removeFromTop(22);
...
void PromptBankPanel::paint(juce::Graphics &g)
{
	...
	g.fillRoundedRectangle(accordionViewport.getBounds().toFloat(), 4.0f);
