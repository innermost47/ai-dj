...
class Autocorrelation
{
  public:
	...
	template <typename T> void acf(const T *R__ in, T *R__ out) const
	{
		for (int i = 0; i < m_m; ++i)
		{
			out[i] = 0.0;
	...
	template <typename T> void acfUnityNormalised(const T *R__ in, T *R__ out) const
	{
...
{
  public:
	FourierFilterbank(int n, double fs, double minFreq, double maxFreq, bool windowed)
	    : m_n(n), m_fs(fs), m_fmin(minFreq), m_fmax(maxFreq), m_windowed(windowed)
	{
...
class ACFCombFilter
{
  public:
	ACFCombFilter(int beatsPerBar, int minlag, int maxlag, double hopsPerSec)
	    : m_beatsPerBar(beatsPerBar), m_min(minlag), m_max(maxlag), m_hopsPerSec(hopsPerSec)
	{
	}
	~ACFCombFilter()
	{
	...
	static void getContributingRange(int lag, int multiple, int &base, int &count)
	{
		if (multiple == 1)
		{
			base = lag;
...
class MiniBPM::D
{
  public:
	...
	template <typename S, typename T> void copy(T *R__ t, const S *R__ s, const int n)
	{
		for (int i = 0; i < n; ++i)
			t[i] = s[i];
	}
	template <typename T> void zero(T *R__ t, const int n)
	{
		for (int i = 0; i < n; ++i)
			t[i] = T(0);
	}
	template <typename T> void unityNormalise(T *R__ t, const int n)
	{
	...
	D(float sampleRate)
	    : m_minbpm(55), m_maxbpm(190), m_beatsPerBar(4), m_inputSampleRate(sampleRate), m_lfmin(0), m_lfmax(550),
	      m_hfmin(9000), m_hfmax(9001), m_input(0), m_partial(0), m_partialFill(0), m_frame(0), m_lfprev(0), m_hfprev(0)
	{
		int lfbinmax = 6;
		m_blockSize = (m_inputSampleRate * lfbinmax) / m_lfmax;
		m_stepSize = m_blockSize / 2;

		m_lf = new FourierFilterbank(m_blockSize, m_inputSampleRate, m_lfmin, m_lfmax, true);

		m_hf = new FourierFilterbank(m_blockSize, m_inputSampleRate, m_hfmin, m_hfmax, true);
	...
	double specdiff(const double *a, const double *b, int n)
	{
		double tot = 0.0;
		for (int i = 0; i < n; ++i)
		{
...
MiniBPM::MiniBPM(float sampleRate) : m_d(new D(sampleRate))
{
}
...
void MiniBPM::setBPMRange(double min, double max)
{
	m_d->m_minbpm = min;
	m_d->m_maxbpm = max;
}

void MiniBPM::getBPMRange(double &min, double &max) const
{
	min = m_d->m_minbpm;
	max = m_d->m_maxbpm;
}

void MiniBPM::setBeatsPerBar(int bpb)
{
	m_d->m_beatsPerBar = bpb;
}

int MiniBPM::getBeatsPerBar() const
{
	return m_d->m_beatsPerBar;
}

double MiniBPM::estimateTempoOfSamples(const float *samples, int nsamples)
{
	return m_d->estimateTempoOfSamples(samples, nsamples);
}

void MiniBPM::process(const float *samples, int nsamples)
{
	m_d->process(samples, nsamples);
}

double MiniBPM::estimateTempo()
{
	return m_d->estimateTempo();
}

std::vector<double> MiniBPM::getTempoCandidates() const
{
