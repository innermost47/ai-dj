	...
	class SettingsComponent : public Component
	{
	  public:
		SettingsComponent(StandalonePluginHolder &pluginHolder, AudioDeviceManager &deviceManagerToUse,
		    ...
		    : owner(pluginHolder),
		      deviceSelector(deviceManagerToUse, 0, maxAudioInputChannels, 0, maxAudioOutputChannels, true,
...
class StandaloneFilterWindow : public DocumentWindow, private Button::Listener
{
  public:
	...
	StandaloneFilterWindow(const String &title, Colour backgroundColour,
	                       std::unique_ptr<StandalonePluginHolder> pluginHolderIn)
	    : DocumentWindow(title, backgroundColour, DocumentWindow::minimiseButton | DocumentWindow::closeButton),
	      pluginHolder(std::move(pluginHolderIn)), optionsButton("Options")
	{
	...
	*/
	StandaloneFilterWindow(const String &title, Colour backgroundColour, PropertySet *settingsToUse,
	    ...
	    : StandaloneFilterWindow(title, backgroundColour,
	                             std::make_unique<StandalonePluginHolder>(
	                                 settingsToUse, takeOwnershipOfSettings, preferredDefaultDeviceName,
	                                 preferredSetupOptions, constrainToConfiguration, autoOpenMidiDevices))
	{
	}
	...
	{
		auto *processor = dynamic_cast<DjIaVstProcessor *>(getAudioProcessor());
		if (!processor)
			return;

		auto defaultFolder = StateManager::getDefaultSessionsFolder();
		fileChooser = std::make_unique<juce::FileChooser>("Save Session", defaultFolder, "*.obsidian");
		...
		fileChooser->launchAsync(juce::FileBrowserComponent::saveMode | juce::FileBrowserComponent::canSelectFiles,
		                         [this, processor](const juce::FileChooser &fc)
		                         {
			                         ...
			                         if (file != juce::File{})
				                         processor->getStateManager().saveToFile(file);
			                         fileChooser.reset();
		                         });
	}

	void customLoadState()
	{
		auto *processor = dynamic_cast<DjIaVstProcessor *>(getAudioProcessor());
		if (!processor)
			return;
